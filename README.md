...
A search is sent concurrently to every configured catalogue adapter. Modrinth works without credentials. Official builds can include an app-specific CurseForge API key only after Overwolf approves that distribution model for Minecraft Server Manager and the repository release workflow is configured with the `CURSEFORGE_API_KEY` Actions secret. This follows the application-key pattern used by Prism Launcher: requests go directly from the desktop app, and an embedded application key must be considered extractable rather than treated as a private user secret. A user may instead enter their own separately approved developer key as a local override; it is validated directly with CurseForge and stored only in Windows Credential Manager for the current Windows account. This is developer API access, not a CurseForge website login or access to a personal CurseForge library.

The app-specific key is never committed to the repository or written to profiles, logs, or manifests; if configured for an approved official build, it is injected from the GitHub Actions secret into that build. Personal overrides are never compiled into the app, installer, or update packages. Each provider reports its own availability, so a missing key or one provider outage does not hide results from another. Searches can be filtered by mod/plugin type and category, while every compatible published version remains selectable rather than assuming the newest file is always correct.

Selecting **Add to draft** resolves the whole plan once and automatically adds every compatible required dependency with the selected item. If the chosen project version has an impossible dependency chain, the builder checks its other compatible published versions and can offer an explicit one-click alternative; it never silently downgrades. Search results stay in place after selection and are marked **In draft**, preventing accidental duplicate additions without losing the user's scroll position. Compatible older dependency versions can be selected, cycles are bounded, optional dependencies remain notices, and declared incompatibilities block the addition. Provider environment and loader metadata determine whether each item belongs on the client, server, both, or needs manual placement review. In particular, client-only content is not copied to the server automatically.

After reviewing the draft, **Build reviewed output** can create a new pack in the manager's persistent `Instances` folder or another selected folder. Immediately before download, the app rebuilds the dependency graph from fresh provider metadata and stops for a new review if required versions or placement changed. It downloads only through each provider's supported route, follows only bounded HTTPS redirects between approved CurseForge CDN hosts, verifies every file against its published hash and size, and stages the result before atomically committing a new folder. Shared mods are copied to both sides, client-only content stays out of the server, plugins stay server-only, required dependencies follow their resolved placement, and an auditable JSON manifest records the exact provider versions and hashes. Existing output folders are never overwritten.

Vanilla, Fabric, Quilt, Forge, and NeoForge choices now resolve an exact version from the platform's official catalogue. For a supported server target, the builder installs that exact baseline inside the same rollback-safe staging operation, records the recommended Java line, imports the completed `Server` folder as a normal machine-tuned profile, and opens the existing explicit EULA review. Any staged `eula=true` is rejected and rolls the output back. Paper and experimental hybrid servers remain clearly labelled **content-only** until equally safe baselines exist; providers without a supported API or permitted download route are reported instead of being scraped.

For a client output, the app offers an explicit second step to install the official **Prism Launcher portable** build and add the client as a separate managed instance. The release metadata and Windows portable archive come from Prism Launcher's official GitHub release, the archive is checked against GitHub's published SHA-256 digest, and extraction is bounded against path traversal, links, oversized archives, and partial installs. Prism's accounts, instances, libraries, and settings stay under `%LocalAppData%\Kidda.MinecraftServerManager\Launcher\Prism`; the normal Minecraft Launcher and `%APPDATA%\.minecraft` are not read or changed. Microsoft sign-in occurs only inside Prism Launcher. Existing managed instances are never overwritten when another build is added.
...
CurseForge access is available from **Build a pack → Sources & accounts** when an official build contains a separately approved app-specific key or after CurseForge approves a user's own developer application. **Prepare application** gives the project owner or personal applicant suggested wording that asks Overwolf to approve the relevant distribution model and the minimal local audit manifest; every answer must be reviewed for accuracy before submission. Without either key, CurseForge is reported as unavailable and Modrinth continues working normally. Removing a personal override returns to app-specific access when it is present.
