...
A search is sent concurrently to every configured catalogue adapter. Modrinth is the first active adapter; each provider reports its own availability so one outage does not hide results from another future provider. Searches can be filtered by mod/plugin type and category, while every compatible published version remains selectable rather than assuming the newest file is always correct.

Before an item enters the draft, required dependencies are resolved recursively, compatible older dependency versions can be selected, cycles are bounded, optional dependencies are left for review, and declared incompatibilities block the addition. Provider environment and loader metadata determine whether each item belongs on the client, server, both, or needs manual placement review. In particular, client-only content is not copied to the server automatically.

This foundation deliberately does not download, install, export, or launch-test a pack yet. Those operations will be added behind the existing review boundary after provider and resolver behaviour has been tested more broadly. The future installed output should produce an exportable client instance and compatible dedicated-server instance while preserving source attribution, licences, distribution restrictions, Minecraft ownership, and authentication requirements. Providers without a safe supported API or permitted download route will be reported clearly instead of being scraped.
