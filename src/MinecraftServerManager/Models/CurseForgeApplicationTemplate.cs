...
public static class CurseForgeApplicationTemplate
{
    ...
    [
        ...
        new(
            ...
            "Use is low-volume and interactive on each Windows installation. There is no background catalogue polling: searches and compatible-version requests occur only after a user action, and selected files are downloaded once for that user's local pack. Search results are not kept as a persistent catalogue cache. The app plans to retain a small local audit manifest after a download containing the selected provider, project and version identifiers, file names, sizes, and published hashes. Because the API terms restrict saving or caching API data, this application explicitly asks Overwolf to confirm whether that minimal per-user audit manifest is permitted; it will be constrained or removed for CurseForge content if required."),
        new(
            "Official-build API key model and additional notes",
            "Minecraft Server Manager requests one app-specific CurseForge API key for its official open-source release builds, comparable to Prism Launcher's documented application-key model. Requests go directly from the user's PC; there is no central API proxy or shared account service. The release workflow reads the approved value from a private GitHub Actions secret and injects it only into official binaries. Because any key compiled into a desktop application can be extracted, the app-specific key must be treated as an application identifier rather than a user secret, and it will not be enabled unless Overwolf explicitly approves that distribution model for this project. A user may instead provide their own separately approved key as a local override stored only in Windows Credential Manager; personal overrides are never packaged, logged, or committed. Please confirm the permitted distribution, download, attribution, quota, and local audit-manifest requirements before app-wide access is enabled.")
    ];
